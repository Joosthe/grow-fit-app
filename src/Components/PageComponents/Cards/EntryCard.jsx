...
export default function EntryCard({ entry }) {
  ...
  const { data: workout } = useStaticCmsData({}, getWorkoutTitleQuery(entry.workoutId));
  return (
    <div className="entrycard__wrapper my-2 px-2 w-full overflow-hidden md:w-1/2 lg:w-1/3 xl:w-1/4">
      <article>
        ...
        <div className="entrycard__body">
          <p className="entrycard__descr">{entry.info.substring(0, 200)}</p>
          <p className="entrycard__score">{entry.score} / 10</p>
        </div>
      </article>
    </div>
  )
